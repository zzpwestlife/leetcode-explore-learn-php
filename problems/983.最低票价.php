<?php
/*
 * @lc app=leetcode.cn id=983 lang=php
 *
 * [983] 最低票价
 *
...
class Solution
{
    ...
    {
        $dp = array_fill(0, end($days) + 1, 0);
        for ($i = 1; $i <= end($days); ++$i) {
            if (in_array($i, $days)) {
                $dp_7 = ($dp[$i - 7] ?? 0) + $costs[1];
                $dp_30 = ($dp[$i - 30] ?? 0) + $costs[2];
                $dp[$i] = min($dp[$i - 1] + $costs[0], $dp_7, $dp_30);
            } else {
                $dp[$i] = $dp[$i - 1];
            }
        }
