...
class Solution
{
    ...
    function findInMountainArray($target, $mountainArr)
    {
        ...
        if ($n == 0) return -1;
        $peakIndex = $this->getMax($mountainArr, 0, $n - 1);
        if ($mountainArr->get($peakIndex) == $target) return $peakIndex;
        $ans = $this->binarySearch($mountainArr, 0, $peakIndex - 1, $target);
        if ($ans != -1) return $ans;

        return $this->binaryReverseSearch($mountainArr, $peakIndex + 1, $n - 1, $target);
    }

    private function getMax($mountainArr, $left, $right)
    {
        // [left,...,right] 区间内寻找，区间至少包含两个元素
        while ($left < $right) {
            // 右中点
    ...
    private function binarySearch($mountainArr, $left, $right, $target)
    {
        // [left,...,right] 区间内寻找，区间至少包含两个元素
        while ($left < $right) {
            // 右中点
    ...
    private function binaryReverseSearch($mountainArr, $left, $right, $target)
    {
        // [left,...,right] 区间内寻找，区间至少包含两个元素
        while ($left < $right) {
            // 右中点
