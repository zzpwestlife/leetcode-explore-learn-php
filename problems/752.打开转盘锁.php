<?php
/*
 * @lc app=leetcode.cn id=752 lang=php
 *
 * [752] 打开转盘锁
 *
...
class Solution
{
    ...
    function openLock1($deadends, $target)
    {
        ...
        while (!$queue->isEmpty()) {
            ...
            for ($i = 0; $i < $count; ++$i) {
                $cur = $queue->dequeue();
                if (in_array($cur, $deadends)) continue;
    ...
    function openLock($deadends, $target)
    {
        ...
        while (!empty($p) && !empty($q)) {
            ...
            foreach ($p as $cur => $v) {
                $visited[$cur] = true;
                // isset 效率远高于 in_array
                if (in_array($cur, $deadends)) continue;
                if (isset($q[$cur])) return $step;
                for ($j = 0; $j < 4; ++$j) {
                    $up = $this->up($cur, $j);
                    if (!isset($visited[$up])) {
                        $temp[$up] = true;
    ...
    public function up($cur, $index)
    {
        $cur = $cur . '';
        if ($cur[$index] == '9') {
            $cur[$index] = '0';
        } else {
            $cur[$index] = (int) ($cur[$index]) + 1;
        }

        return $cur;
    }

    public function down($cur, $index)
    {
        $cur = $cur . '';
        if ($cur[$index] == '0') {
            $cur[$index] = '9';
...
$target = "4894";
// echo (new Solution())->openLock1($deadends, $target) . PHP_EOL;
